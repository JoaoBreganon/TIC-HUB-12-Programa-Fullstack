<template>
  <div class="min-h-screen bg-linear-to-br from-indigo-50 via-white to-slate-50 text-slate-900">
    <header class="max-w-6xl mx-auto px-4 py-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-indigo-100">
      <div>
        <h1 class="text-2xl md:text-3xl font-bold tracking-tight text-indigo-900">TechStore V5</h1>
      </div>
      ...
      <div class="flex flex-wrap gap-2 text-sm items-center">
         <template v-if="auth.isAuthenticated">
           <div class="flex items-center gap-2 bg-indigo-50 rounded-full px-4 py-1.5 border border-indigo-100">
             <i class="pi pi-user text-indigo-600"></i>
             <span class="font-semibold text-indigo-700">{{ auth.userName }}</span>
             <span class="text-xs text-indigo-400 uppercase font-bold">({{ auth.userRole }})</span>
           </div>
           <Button size="small" label="Sair" icon="pi pi-sign-out" severity="danger" outlined @click="handleLogout" />
         </template>
         <template v-else>
           <RouterLink to="/login">
             <Button size="small" label="Entrar" icon="pi pi-sign-in" severity="info" />
           </RouterLink>
         </template>
      </div>
    </header>
